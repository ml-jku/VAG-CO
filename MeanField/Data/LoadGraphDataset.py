...
class SolutionDatasetLoader:
    def __init__(self, config = {}, dataset="MIS", problem="MIS", batch_size=32, relaxed=False, seed=123):
    ...
    def dataloaders(self):
        ...
        try:
            ...
            dataset_test = None
            print("No test dataset was found")
...
class SolutionDataset(Dataset):
    def __init__(self, config = {}, dataset="ENZYMES", problem="MIS", mode="val", relaxed=False, seed=123):
        ...
        if self.relaxed:
            self.path = config["working_directory_path"] + "/DatasetCreator/loadGraphDatasets/DatasetSolutions/no_norm/"

            self.jraph_list, (self.normed_energies, self.gs_bin_states, self.val_mean_energy,
                              self.val_std_energy) = self.__load_dataset()

        else:
            self.path = config["working_directory_path"] + "/DatasetCreator/loadGraphDatasets/DatasetSolutions/normed_H_graph_sparse/"
